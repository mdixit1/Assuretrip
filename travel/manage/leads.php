...
if(isset($_SESSION['amail']) && isset($_SESSION['aid']) && isset($_SESSION['apass'])){
	...
	if(count($userd)){
		foreach($userd as $usr){
			...
			$update_lead = $db->prepare("UPDATE leads SET lead_notf='1'");
			$update_lead->execute();
        ...
        <?php 
			if(isset($_GET['transfer'])){ 
				...
				if(isset($_POST['find'])){
					...
					$_SESSION['ldesc'] = $_POST['ldesc'];
					header('Location:leads.php?transfer=$luniqid');
				}
				
			  if(isset($_POST['transfer'])){
				  $ldesc = $_SESSION['ldesc'];
					foreach($_POST['agid'] as $val){
							if($val!=''){
								$add_leads = $db->prepare("INSERT INTO lead_transfer(traf_agid,traf_leadid,transfer_status,transfer_desc,transfer_date)VALUES(:val, :foundlid, '1', :ldesc, :date)");
								$add_leads->bindParam(':val',$val);
								$add_leads->bindParam(':foundlid',$foundlid);
								$add_leads->bindParam(':ldesc',$ldesc);
		...
		?>
            ...
            <div class="fulldv leadfind">
                <form method="post">
                    ...
                    <div class="col-md-3">
                    	<textarea name="ldesc" id="" cols="30" rows="10" required/></textarea>
                    </div>
