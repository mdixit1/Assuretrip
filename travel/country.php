	...
	$(function(){
		$('.item_filter').click(function(){
			$('.product-data').html('<div id="loaderpro" style="" ></div>');
			 cont   = multiple_values('cont');
			$.ajax({
				url:"<?php echo $url; ?>ajax1.php",
				type:'post',
				data:{cont:cont},
...
<div class="see-section wrapper">
  ...
  <?php if(isset($_GET['contry'])){
			$cntname = $_GET['contry'];
			$found_name = $db->prepare("SELECT country_id,country_name FROM country WHERE country_url=:cntname");
			$found_name->bindParam(':cntname',$cntname);
			$found_name->execute();
			$findd = $found_name->fetchAll();
			if(count($findd)){
				foreach($findd as $find){
					$cntid = $find['country_id'];
					$_SESSION['cntid'] = $cntid;
						$count_contpck = $db->prepare("SELECT COUNT(package_id) FROM packages WHERE cntid=:cntid");
						$count_contpck->bindParam(':cntid',$cntid);
						$count_contpck->execute();
						$cntck = $count_contpck->fetchColumn();
  ?>	  
  		  
    <h1 style="text-align:center;"><?php echo $find['country_name']; ?></h1>
    <div class="section honeymoon_sect">
    	<div class="container">
        	<div class="row">
                            ...
                            <div class="panel panel-default">
                                ...
                                <div id="filtertab1" class="panel-collapse collapse in" role="tabpanel" aria-labelledby="headingOne">
                                    <div class="panel-body acco_body">
                                    	<ul>
                                         <?php
										 	$show_contry = $db->prepare("SELECT * FROM country");
											$show_contry->execute();
											$allcntry = $show_contry->fetchAll();
											foreach($allcntry as $alcnt){
										?>	
                                        	<li><input type="checkbox" name="#" class="item_filter cont" value="<?php echo $alcnt['country_id']; ?>"><?php echo $alcnt['country_name']; ?></li>
                                         <?php } ?>   
                                        </ul>
                                    </div>
                                </div>
			   ...
			   <?php
			   	  $cat_package = $db->prepare("SELECT pk.*,stt.state_name FROM packages pk JOIN state stt ON stt.state_id=pk.stid WHERE pk.cntid=:cntid LIMIT $start, $perpage");
				  $cat_package->bindParam(':cntid',$cntid);
				  $cat_package->execute();
