...
$ste = isset($_REQUEST['ste'])?$_REQUEST['ste']:"";
...
<body>
                ...
                <?php
					   if($ste !=''){
							$skillsdata =implode("','",$ste);  
					     	$query .= "SELECT pk.*,stt.state_name FROM packages pk JOIN state stt ON stt.state_id=pk.stid WHERE pk.stid IN('$skillsdata')"; 
					   }
					   else{
						  $query .= "SELECT pk.*,stt.state_name FROM packages pk JOIN state stt ON stt.state_id=pk.stid WHERE pk.cntid='$stid'"; 
					    }
