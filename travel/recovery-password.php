...
if((isset($_GET['mail'])) && (isset($_GET['code']))){
	$reccode = $_GET['code'];
	$recmail = $_GET['mail'];
	$checkdetail = $db->prepare("SELECT agpass_code FROM agent_registration WHERE agpass_code=:reccode AND agent_mail=:recmail");
	...
	if(count($countdetail)){
		...
		if(isset($_POST['change'])){
			...
			if(!($newpassword == $repassword)){
				...
				$updatepassword = $db->prepare("UPDATE agent_registration SET agent_pass=:newpassword, agpass_code='0' WHERE agpass_code=:reccode AND agent_mail=:recmail");
